module.exports = async (id, prefix, axios, token) => {
    ...
    if(isNaN(id)){
        res = await axios.get(`${prefix}/movie/${id}/quote`, {headers: {Authorization: `Bearer ${token}`}}).catch(err => {
    ...
    if(!data || data.length < 1) throw new Error(`An error occurred, possible invalid movie ID.`)
