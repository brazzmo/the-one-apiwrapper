module.exports = async (id, prefix, axios, token) => {
    ...
    if(isNaN(id)){
        res = await axios.get(`${prefix}/quote/${id}`, {headers: {Authorization: `Bearer ${token}`}}).catch(err => {
            if(err.response.status === 401) throw new Error('Invalid Token Provided.')
        })

        data = res.data.docs[0];
    } else {
        const getter = await axios.get(`${prefix}/quote`, {headers: {Authorization: `Bearer ${token}`}}).catch(err => {
            if(err.response.status === 401) throw new Error('Invalid Token Provided.')
        })
